...
def _reason_row(source: str, target: str, kind: str, *, entity_id: str = "", relation_id: str = "", notes: str = "") -> dict[str, str]:
    discriminator = entity_id or relation_id or notes or kind
    return {
...
def derive_reasons(
    ...
    *,
    ...
    appearances_by_entity: dict[str, set[str]] = defaultdict(set)
    for row in appearances:
        entity_id = (row.get("entity_id") or "").strip()
        work_id = (row.get("work_id") or "").strip()
        status = (row.get("verification_status") or "").strip()
        if entity_id and work_id and status != "rejected":
            appearances_by_entity[entity_id].add(work_id)

    reasons: list[dict[str, str]] = []

    if mode != "explicit_only":
        for entity_id in sorted(appearances_by_entity):
            ordered_works = sorted(appearances_by_entity[entity_id], key=lambda w: work_order.get(w, ("9999-99-99", w)))
            ...
            for a, b in candidate_pairs:
                ...
                reasons.append(_reason_row(source, target, "shared_entity", entity_id=entity_id))

        if include_variants:
            components = _variant_components(entity_relations)
            grouped: dict[str, dict[str, set[str]]] = defaultdict(lambda: defaultdict(set))
            for entity_id, work_ids in appearances_by_entity.items():
                root = components.get(entity_id)
                if root:
                    grouped[root][entity_id].update(work_ids)
                    ...
                    for a in sorted(entity_map[e1]):
                        for b in sorted(entity_map[e2]):
                            ...
                            if mode == "target_centric" and target != target_work_id:
                                continue
                            reasons.append(_reason_row(source, target, "variant_entity", entity_id=f"{e1}|{e2}", notes="variant_of relation explicitly enabled"))
    ...
    if mode in {"explicit_only", "combined_all_pairs"}:
        for row in explicit_relations:
            ...
            reasons.append(_reason_row(source, target, "explicit_relation", relation_id=relation_id, notes=notes))

    # Stable exact de-duplication by reason_id, independent of input order.
...
def write_derived_edges(repo_root: Path, mode: str = "combined_all_pairs") -> dict[str, int]:
    ...
    _write_csv(derived / "work_pair_reasons.csv", reasons, ["reason_id", "source_work_id", "target_work_id", "reason_kind", "entity_id", "relation_id", "notes"])
