...
- 公開UIの予習プランは **サイト提案ルート** と **完全版** の2つに整理しました。
- 公式予習ルートの登録データ・出典・監査情報はデータベースと静的エクスポートに保持しますが、公開UIの選択肢・点灯ボタンには表示しません。
- チャートの独立した「点灯範囲」設定をなくし、接続重要度と選択条件から表示を決めます。過去の共有状態に残る公式モードはサイト提案ルートへ正規化します。

## v5.21.2 — 時系列選択モードの監査修正

- 時系列線の点灯を、チャートの **関連全体 / 1つ前のみ / OR / AND / PATH** と同じ選択状態から判定するようにしました。
- デスクトップSVGとモバイルCanvasで同じ分類ロジックを共有し、独立作品の表示用線は探索対象から除外しました。
- FOX X-MENの分岐線に端点を明示し、公開順ビューや199本の関係地図エッジは変更していません。

## v5.21.1 — 時系列線をゴール選択に連動

- ③「世界線・時系列」の明示的な系列線を、選択したゴールの前史・後続に合わせて点灯するようにしました。
- 時系列線は①「関係地図」の199接続とは別の表示用系列エッジとして扱い、短絡関係を新たに作りません。
- ②「公開順」は公開日・時代区分を読むビューとして維持し、作品同士の関係矢印は追加していません。

## v5.21.0 — 予習プランの3モード分離

- チャートの点灯範囲を **関連全体** と **1つ前のみ**（選択作品へ直接入る前史）に整理しました。
- 予習プランの公開選択肢を **公式予習ルート**、**サイト提案ルート**、**完全版** の3つに分離しました。
- **公式予習ルート** は、根拠URLと監査レビューが登録された公式ルートだけを表示します。未登録の場合にサイト提案へ自動切替しません。
- **サイト提案ルート** は、公式ルートとは分離し、監査済み接続表とサイト提案ルートから中核の前史を表示します。
- **完全版** は、中核・推奨の前史を再帰的に辿り、ゴールへ直接入る参照接続も補助的に加えます。公式／サイト提案ルートを混ぜず、参照接続を新たな探索の踏み台にはしません。
- 旧 `minimum` / `recommended` の共有状態は、後方互換のため **サイト提案ルート** として読み込みます。
...
- 公式の作品別予習ルートは、根拠URLと確認日を持つ `data/prewatch_official_routes.json` から静的表示データへ取り込みます。公式未登録の作品は、公式モードでは未登録と明示し、サイト提案モードで別途確認できます。
...
公開ページは、ビルド時に生成された `data/derived/flowchart.json` を相対 URL で一度だけ読み込みます。作品・接続・キャラクター索引の実行時データと、詳細・時系列の表示ポリシーはこの静的 JSON が提供し、ブラウザ側でデータベースを直接開くことはありません。
...
- ゴール選択、複数選択、PATH、人物フィルターは既存 edge のクラス・透明度だけを更新し、EDGES の内容や edge ID を追加・変更しません。
- 接続詳細には `reason_ids` と `reasonsById` から解決した根拠を表示します。スマホの選択経路は既存 Canvas 更新を保ち、フル再構築を行いません。
