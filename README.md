## v5.20.8 — 視聴範囲と予習ルート表記の整理

- チャートの点灯範囲を **関連全体** と **1つ前のみ**（選択作品へ直接入る前史）に整理しました。
- 予習プランの公開選択肢を **おすすめ** と **完全版** に統一しました。旧来の中核限定値は、保存済み共有状態を読むための内部互換値としてのみ残しています。
- **おすすめ** は、根拠URLと監査レビューが登録された公式の作品別予習ルートを優先します。未登録の場合は、**サイト提案ルート** または接続表由来の候補であることを明示して代替表示します。
- **完全版** は、中核・推奨の前史を再帰的に辿り、ゴールへ直接入る参照接続も補助的に加えます。参照接続を新たな探索の踏み台にはしません。
- 「公式予習ルート」「サイト提案ルート」「接続表由来」の出所を予習プラン内に表示し、公式リスト未登録の候補を公式の必須リストと誤認しないようにしました。
...
- 公式の作品別予習ルートは、根拠URLと確認日を持つ `data/prewatch_official_routes.json` から静的表示データへ取り込みます。未登録の作品では、サイト提案ルートまたは接続表由来であることをプラン内に明示します。
