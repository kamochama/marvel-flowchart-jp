## v5.20.7 — 公式予習ルート登録と出典表示

- **最低限（仮）** は、選択したゴールへ直接入る「中核」接続だけを表示します。中核接続をさらに再帰探索して作品数を増やしません。
- **おすすめ** は、根拠URLと監査レビューが登録された公式の作品別予習ルートを優先します。未登録の場合は、監査済み編集ルートまたは接続表由来の候補であることを明示して代替表示します。
- **完全版** は、中核・推奨の前史を再帰的に辿り、ゴールへ直接入る参照接続も補助的に加えます。参照接続を新たな探索の踏み台にはしません。
- 「公式」「編集ルート」「接続表由来」の出所を予習プラン内に表示し、公式リスト未登録の候補を公式の必須リストと誤認しないようにしました。
...
- 公式の作品別予習ルートは、根拠URLと確認日を持つ `data/prewatch_official_routes.json` から静的表示データへ取り込みます。未登録の作品では、編集ルートまたは接続表由来であることをプラン内に明示します。
