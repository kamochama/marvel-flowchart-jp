...
> **Current wave009 note (2026-09-04):** PR #64 is merged into `main` at `227d589917fdf05610bb22f072aaf076151262bb`; it is the current semantic baseline. See §26 for the verified relation-evidence promotion results.
...
> **Current Phase 4 viewer note (2026-09-10):** PR #86 is merged into `main` at `642634d37d6cd91c972b776bb5f9965eeb35c88e`. It completes the bounded search/preparation update contract: no chart rebuilds, no push-history growth, and preserved mobile scroll/focus anchors. See §32 and the handoff's §0.1.18.
...
## 22. Relation evidence promotion wave005 — production baseline (2026-09-03)

PR #55 (`https://github.com/kamochama/marvel-flowchart-jp/pull/55`) integrated the fifth bounded relation-evidence promotion wave into `main` at `cf4da128b5b8783a3d635b48470dac413ac35b1d`. It promotes five existing relations: *Iron Man* → *Iron Man 2*, *Avengers: Infinity War* → *Avengers: Endgame*, *Daredevil* Season 1 → Season 2, *What If...?* Season 1 → Season 2, and *Loki* Season 1 → Season 2. Existing IDs and semantics are preserved; each relation now has an official-source evidence row and a `legacy_seed -> source_verified` review transition. No release/status, chronology, identity, multiverse-transition, or new work-pair fact was inferred. Across→Beyond remains deferred.

Verification at this boundary:

- bundled-Python full suite: `447` tests pass, `4` environment-gated skips;
- build/audit: audit and content-audit issues `0`, SQLite FK rows `0`, `integrity_check=ok`;
...
## 23. Relation evidence promotion wave006 — production baseline (2026-09-03)

PR #58 (`https://github.com/kamochama/marvel-flowchart-jp/pull/58`) integrated the sixth bounded relation-evidence promotion wave into `main` at `ee90bf3afcec53a4d3be2f1ba290c1282a90a31a`. It promotes four existing relations: *Spider-Man: Across the Spider-Verse* → *Beyond the Spider-Verse*, *Spider-Man: Into the Spider-Verse* → *Across the Spider-Verse*, *X-Men: The Animated Series* → *X-Men '97* Season 1, and *Spider-Man: Homecoming* → *Far From Home*. Existing IDs and semantics are preserved; each relation now has relation-specific official-source evidence and a `legacy_seed -> source_verified` review transition. No release/status, chronology, identity, multiverse-transition, or new work-pair fact was inferred.

Verification at this boundary:

- bundled-Python full suite: `449` tests pass, `4` environment-gated skips;
- build/audit: audit and content-audit issues `0`, SQLite FK rows `0`, `integrity_check=ok`;
- relation table: `164` rows (`31` source_verified, `130` legacy_seed, `3` superseded); sources/evidence/reviews `68` / `158` / `132`;
- graph/export compatibility: `131` nodes, `355` edges, `562` reasons, prewatch `199`, story paths `83/83`;
- independent connectivity audit: `pass=39`, `deferred=536`, projection mismatches `0`, reason orphans `0`, unsupported transition edges `0`;
...
## 26. Relation evidence promotion wave009 — production baseline (2026-09-04)

PR #64 (`https://github.com/kamochama/marvel-flowchart-jp/pull/64`) integrated the ninth bounded relation-evidence promotion wave into `main` at `227d589917fdf05610bb22f072aaf076151262bb`. It promotes four existing relations: *Avengers: Endgame* → *Spider-Man: Far From Home*, *WandaVision* → *Doctor Strange in the Multiverse of Madness*, *Ant-Man* → *Ant-Man and the Wasp*, and *X-Men: First Class* → *X-Men: Days of Future Past*. Existing IDs and semantics are preserved; each relation now has relation-specific official-source evidence and a `legacy_seed -> source_verified` review transition. No release/status, chronology, identity, Earth, multiverse-transition, or new work-pair fact was inferred.

Verification at this boundary:

- bundled-Python full suite: `455` tests pass, `4` environment-gated skips;
- build/audit: audit and content-audit issues `0`, SQLite FK rows `0`, `integrity_check=ok`;
...
## 27. Relation evidence promotion wave010 — production baseline (2026-09-04)

PR #66 (`https://github.com/kamochama/marvel-flowchart-jp/pull/66`) integrated the tenth bounded relation-evidence promotion wave into `main` at `696b21b485ee77ce647c5f9206b428afaf84905c`. It promotes eleven existing relations with relation-specific official evidence and `legacy_seed -> source_verified` review transitions; all relation IDs, directions, kinds, certainty values, and graph topology are preserved. The *X-Men: Days of Future Past* → *X-Men: Apocalypse* candidate remains deferred under the strict source-specific boundary.
...
- local real Chrome/CDP selection, interaction, chronology, and publication-order audits pass with zero failures/mismatches and zero synthetic edges;
- hosted GitHub Actions run `33821836422` passed `test` and selection but repeatedly hit Chrome/DevTools startup or selection timeouts in the interaction job; after explicit user authorization, the PR was merged administratively rather than bypassing the failure silently;
...
## 32. Search and preparation update stabilization — production baseline (2026-09-10)

PR #86 (`https://github.com/kamochama/marvel-flowchart-jp/pull/86`) integrated the Phase 4 search/preparation update contract into `main` at merge commit `642634d37d6cd91c972b776bb5f9965eeb35c88e`. Search query updates are now covered by a DOM-only audit with replace-only history; preparation watch, tier, and goal updates preserve the plan viewport and focused item action through projection rerenders. The new plan is `docs/superpowers/plans/2026-09-10-marvel-ui-phase4-search-preparation.md`.

Verification at this boundary:

- bundled-Python full suite: `577` pass, `5` environment-gated skips;
- build/audit/content-audit: `0` issues, story paths `83/83`, prewatch edges `199`, SQLite integrity `ok`, export `131` nodes / `355` edges / `562` reasons;
- real Chrome/CDP selection audit: `131` works × `2` public tiers, `0` exact-set mismatches;
- interaction, chronology, publication-order, and mobile-shell audits: pass (`131` cards, `0` failures, `0` synthetic edges);
- Phase 4 mobile assertions: search focus/scroll preserved, plan anchor preserved, chart rebuild delta `0`, push-history delta `0`;
- hosted GitHub Actions run `34457417548`: all six required jobs pass;
...
GitHub Actions run `34694839654` passed all six required jobs. Pages deployment run `34695230864` succeeded and the public URL returned HTTP `200`.

This is viewer-only. Canonical CSVs, persistent review ledgers, graph and chronology/worldline semantics, work IDs, and the public plan-tier meaning are unchanged. Phase 6 old-layer removal remains a separate planned boundary; no semantic DB phase is implied by this viewer work.

## 34. Phase 6 Task 2/3 retired inspection layer — production baseline (2026-09-12)

PR #92 (`https://github.com/kamochama/marvel-flowchart-jp/pull/92`) first made `#detail` a goal-summary owner and `#sheetHost` the sole desktop inspection owner. PR #93 (`https://github.com/kamochama/marvel-flowchart-jp/pull/93`) then removed only the now-unreachable inspection renderer cluster: `marvelRenderFocusedDetail`, its `sortedDirect`/`attrEsc` helpers, the unreferenced `mobileWorkDetailHtml` alias, and the private `v515-detail-*`/work-link CSS. The chart's `g.node.detail-focus`, `sheetWorkDetailHtml`, search projection, selection/goals, SheetHost, and all canonical data remain active. PR #93 merged into `main` at `959821cd3bc6d8688b635d8ffadfe3185e5017e2`.

The RED-to-GREEN contract is `tests/library_v5/test_phase6_legacy_layer_contract.py::test_phase6_retired_inspection_markup_is_absent`; it explicitly preserves chart `detail-focus` paint and `sheetWorkDetailHtml`. Local verification on the branch passed `588` library-v5 tests (`5` environment-gated skips), deterministic build audit/content-audit issues `0`, story paths `83/83`, SQLite integrity `ok`, and unchanged compatibility counts `prewatch_edges=199`, `work_edges_all=355`, `work_pair_reasons=562`. Real Chrome interaction, exhaustive selection, chronology, publication-order (`131` cards, `failures=0`, `syntheticEdges=0`), and mobile-shell audits all passed. GitHub Actions run `34703781736` passed all six required jobs; Pages deployment run `34704045229` completed successfully and reports `status=built` at `https://kamochama.github.io/marvel-flowchart-jp/`.

This remains viewer-only: canonical CSVs, persistent review ledgers, graph/export semantics, official prewatch data, selection/goal/prewatch meaning, and mobile/desktop state contracts are unchanged. The next bounded Phase 6 boundary is the reachability audit for the remaining `#right` legacy mobile-header fragment and explicit `ACTIVE_ADAPTER` documentation; do not remove search adapters or mixed SheetHost CSS by name alone.
