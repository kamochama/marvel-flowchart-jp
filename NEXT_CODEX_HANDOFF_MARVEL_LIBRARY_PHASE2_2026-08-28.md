...
## 0.1.7 Production baseline after the 2026-09-03 relation evidence wave004

PR #53 (`https://github.com/kamochama/marvel-flowchart-jp/pull/53`) is integrated into `main` at `e1b352a6345de5c2783544ac585a547fdc635f72`. It promotes four existing relations using official Disney or Marvel sources: *Hawkeye* → *Echo* as a spinoff, *Daredevil: Born Again* Season 1 → Season 2 as a sequel, *Daredevil: Born Again* Season 2 → *The Punisher: One Last Kill* as an indirect crossover, and *The Punisher* Season 1 → Season 2 as a sequel. Existing IDs, directions, relation kinds, directness, continuity scope, and certainty values are preserved; each transition is recorded as `legacy_seed -> source_verified` with exact evidence and review rows. No release/status, chronology, identity, multiverse-transition, or new work-pair fact was added.

Fresh verification for the merged PR #53 baseline:

- `445` / `445` library-v5 tests pass locally (`4` environment-gated browser skips);
- bundled-Python build: audit/content-audit issues `0`, SQLite foreign-key rows `0`, `integrity_check=ok`;
...
## 0.1.9 Production baseline after the 2026-09-03 relation evidence wave006

PR #58 (`https://github.com/kamochama/marvel-flowchart-jp/pull/58`) is integrated into `main` at `ee90bf3afcec53a4d3be2f1ba290c1282a90a31a`. It promotes four existing relations with relation-specific official evidence and review transitions: *Spider-Man: Across the Spider-Verse* → *Beyond the Spider-Verse*, *Spider-Man: Into the Spider-Verse* → *Across the Spider-Verse*, *X-Men: The Animated Series* → *X-Men '97* Season 1, and *Spider-Man: Homecoming* → *Far From Home*. Existing IDs, directions, relation kinds, directness, continuity scope, and certainty values are preserved; no release/status, chronology, identity, multiverse-transition, or new work-pair fact was added.

Fresh verification for the merged PR #58 baseline:

- `449` / `449` library-v5 tests pass locally (`4` environment-gated browser skips);
- bundled-Python build: audit/content-audit issues `0`, SQLite foreign-key rows `0`, `integrity_check=ok`;
...
## 17. Phase 6 Task 2/3 presentation ownership and dead-helper cleanup — 2026-09-12

PR #92 (`https://github.com/kamochama/marvel-flowchart-jp/pull/92`) removed desktop inspection's duplicate `#detail` writer and mirror ownership while preserving `#detail` for goal/reset summary and `#sheetHost` for inspection. PR #93 (`https://github.com/kamochama/marvel-flowchart-jp/pull/93`) removed the proven-dead `marvelRenderFocusedDetail` renderer, `sortedDirect`, `attrEsc`, `mobileWorkDetailHtml`, and its private v515 detail CSS. The chart `detail-focus` paint, `sheetWorkDetailHtml`, mobile search adapter, selection/goal semantics, and canonical DB inputs remain protected. The resulting production `main` merge commit is `959821cd3bc6d8688b635d8ffadfe3185e5017e2`.

Fresh verification: `588` bundled-runtime unit tests passed with `5` environment-gated skips; deterministic build reports `audit_issue_count=0`, content-audit issues `0`, story paths `83/83`, SQLite `integrity_check=ok`, `prewatch_edges=199`, `work_edges_all=355`, and `work_pair_reasons=562`. Real Chrome/CDP interaction, selection, chronology, publication-order (`131` cards, `failures=0`, `syntheticEdges=0`), and mobile-shell audits passed. GitHub Actions run `34703781736` is fully GREEN. Pages deployment run `34704045229` succeeded and GitHub Pages reports `status=built` for `https://kamochama.github.io/marvel-flowchart-jp/`.

No canonical CSV, persistent review ledger, graph/export semantic, official prewatch, selection/goal, or public tier meaning changed. The next bounded work is Phase 6's remaining `#right` legacy-header reachability audit and explicit `ACTIVE_ADAPTER` inventory; search projection and shared SheetHost paths must remain protected until a separate ownership proof exists. Ordinary ChatGPT was consulted during the plan and final cleanup review; its recommendation was applied conservatively.
