# マーベル作品相関図 日本版 — PUBLIC v5.15.1 監査
...
- スマホタップ: 関連ラインを一時表示するが、詳細シートは自動では開かない。チャート上に留まり、選択作品のコンパクトバーを表示する。
- スマホのコンパクトバー: `詳細を見る` で詳細シートを明示的に開き、`🎯 ゴール` でその作品をゴールへ追加 / 解除できる。
...
- スマホタップ関係表示 / 詳細シート非自動表示: PASS
- スマホ選択バー `詳細を見る` / `🎯 ゴール`: PASS
- PC横詳細パネル維持: PASS
