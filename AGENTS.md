...
The latest integrated production/audit baseline after PR #58 is:

- `main` `ee90bf3afcec53a4d3be2f1ba290c1282a90a31a` (merge commit for PR #58)
- PR #58 required checks: GREEN (`test`, `browser-selection-audit`, `browser-interaction-audit`, `browser-chronology-audit`, and `browser-publication-order-audit`; the first publication-order attempt emitted no harness JSON and the failed job rerun passed)
- Pages deployment run `33759982789` for the wave006 baseline succeeded; the public site and generated JSON returned HTTP `200`.
- 449 / 449 library-v5 unit tests PASS locally (4 browser tests are environment-gated skips in the ordinary suite)
...
- work relations: 164 rows (31 `source_verified`, 130 `legacy_seed`, 3 `superseded`)
- sources: 68
- evidence: 158
- reviews: 132
- `work_edges_all`: 355
